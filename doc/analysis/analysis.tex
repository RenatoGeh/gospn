...
formalize the notion of density.

\begin{definition}[$\varepsilon$-neighborhood]
  Let $e_1\in\set{D}$ be an element and $d:\set{D}\times\set{D}\to \mathbb{R}$ the euclidean
  distance between two elements. The $\varepsilon$-neighborhood of $e_1$, denoted by
  $N_\varepsilon(e_1)$ is defined as $N_\varepsilon(e_1)=\{e_2\in\set{D}|d(e_1,
  e_2)\leq\varepsilon\}$.
\end{definition}

The $\varepsilon$-neighborhood of an element $e$ is simply the set of all elements different than
$e$ that are at least at $\varepsilon$ of distance from $e$. We refer to \textit{core element} as
...
\begin{definition}[directly density-reachable]
  ...
  $\varepsilon$ and $\mu$ if $e_1\in N_\varepsilon(e_2)$ and $|N_\varepsilon(e_2)|\geq\mu$.
\end{definition}

The parameters $\varepsilon$ and $\mu$ define whether two points are close enough to be considered
part of a cluster. Parameter $\varepsilon$ defines the ``area'' to be examined and $\mu$ defines
